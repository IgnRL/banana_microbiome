...
eu.permanova <- adonis2(eu.distances ~ time*treatment, data = data.t0.t5, permutations = h) %>% data.frame()
...
pca.plot.t0.t5.time <- ggplot(t0.t5.pca, aes(PC1, PC2, col= treatment, shape= time))+
  ...
  stat_ellipse(aes(group =time, fill= time), level = 0.95,geom= "polygon", alpha = 0.3, show.legend = FALSE)+
  scale_fill_manual(values= c("gold", "gray50"))+
  geom_polygon(aes(group= interaction(time, treatment),col= treatment), alpha= 0, show.legend = TRUE)+
  theme_classic2()
pca.plot.t0.t5.time

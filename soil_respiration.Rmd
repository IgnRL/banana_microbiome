...
plot_induced_t2 <- respirationt2 %>% ggplot(aes(x= factor(treatment, level= treat.order), y= mg_co_induced, col=treatment, group=treatment))+
  ...
  scale_x_discrete(labels= treat.labels)+
  labs(subtitle = "" , caption = "Different letters indicate statistically significant differences between each treatment at T2.\nCrossbar indicate the average of the three samples ")+
  ggtitle("Basal respiration at T2", subtitle= paste("ANOVA (HSDTukey) p-value: ", format(pvalue_anova_inducedt2, scientific= TRUE, digits=2 ), sep= ""))+
  theme_classic2()
plot_induced_t2
...
ggsave(respiration_basal_all, file="soil_respiration_result/all_basal_respiration.png", width= 1900,height= 1200, units= "px")

respiration_induced_all <- ggarrange(plot_induced_t0 + theme(legend.position="none"), plot_induced_t1 + theme(legend.position="none"), plot_induced_t2 + theme(legend.position="none"), plot_induced_t3+ theme(legend.position="none"), plot_induced_t4 + theme(legend.position="none") , ncol = 3)
...
model_respibasalt0t4 <- lmer(mg_co_basal~ time*treatment + (1|treat.pot), data=soil_resp)
summary(model_respibasalt0t4)

anova_model_respibasalt0t4 <- anova(model_respibasalt0t4)
anova_model_respibasalt0t4
...
basalt0t4contras <- data.frame(emm_respibasalt0t4$contrasts)#data frame para sacar el valor del contraste 
basalt0t4contras
...
basalt0t4_time <- respirationt0t4mean %>% ggplot(aes(x=factor(treatment, level= treat.order), y= mg_co_basal_mean))+
  ...
  labs(subtitle = "" , caption = "Different letters indicate statistically significant differences between Time (T0 and T4).\nMean and SD of each treatment")+
  ggtitle("Interaction between basal respiration at T0 and T4", subtitle= "ANOVA (Tukey-Interac-Time) p-value: 2.10e-09 \nEmmeans model") +
...
plotdelta_basalt0t4 <- delta_basalt0t4 %>% ggplot(aes(x=factor(treatment, level= treat.order), y= delta, fill= treatment))+
  ...
  scale_x_discrete(labels= treat.labels)+
  labs(subtitle = "" , caption = "Different letters indicate statistically significant differences \nbetween delta variation in each treatment between T0 and T4")+
  ggtitle("Δ Basal Respiration basal at T0 and T4", subtitle= "ANOVA Δ (Tukey) p-value : 0.01512\nEmmeans model")+
  theme_classic2()
plotdelta_basalt0t4 
...
anova_inducedt0t4int <- aov(mg_co_induced ~ time * treatment + rep, data=respirationt0t4) # interaction between them? # p-value 0.1037
summary(anova_inducedt0t4int)

inducedt0t4lm <- lm(mg_co_induced~ time*treatment +rep, data=respirationt0t4)#model
anova(inducedt0t4lm)
...
inducedt0t4_time <- respirationt0t4mean %>% ggplot(aes(x=factor(treatment, level= treat.order), y= mg_co_induced_mean))+
  ...
  theme_classic2()+
  labs(subtitle = "" , caption = "Different letters indicate statistically significant differences between Time (T0 and T4).\nMean and SD of each treatment")+
  ggtitle("Interaction between induced respiration at T0 and T4", subtitle= "ANOVA (Tukey-Interac-Time) p-value:0.1037 de time ??? \nEmmeans model") +
  scale_x_discrete(labels= treat.labels)+
  geom_text(data = pval_inducedt0t4_time, aes(x= treatment, y= emmean, label= sign), nudge_x= 0, nudge_y= 25)+
  geom_errorbar(data=respirationt0t4mean, aes(ymin=mg_co_induced_mean-mg_co_induced_sd, ymax=mg_co_induced_mean+mg_co_induced_sd, group= time), position=position_dodge(0.9), size =0.2, width=0.2)
...
anova(delta_inducedt0t4_lm) #pval 0.002363
...
plotdelta_inducedt0t4 <- delta_inducedt0t4 %>% ggplot(aes(x=factor(treatment, level= treat.order), y= delta, fill= treatment))+
  ...
  scale_x_discrete(labels= treat.labels)+
  labs(subtitle = "" , caption = "Different letters indicate statistically significant differences \nbetween delta variation in each treatment between T0 and T4")+
  ggtitle("Δ Respiration induced at T0 and T4", subtitle= "ANOVA Δ (Tukey) p-value : 0.002363\nEmmeans model")+
  theme_classic2()

plotdelta_inducedt0t4
induced_t0t4plot <- ggarrange(inducedt0t4_time, plotdelta_inducedt0t4+ guides(fill= "none") , ncol = 2)

#induced_t0t4plot <- annotate_figure(induced_t0t4plot, top = text_grob("Induced respiration", color = "black", face = "bold", size = 10),bottom = text_grob("Interaction and variation between beginning and end of the experiment", size=10))

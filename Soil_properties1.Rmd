...
Vamos analizar diferencias en el primer muestreo (to), por ello usamos la funcion subset que es un condicional como ($) para elegir determinadas variables dentro de un objeto, subset(soil_prop, soil_prop$time== "t0") significa solo analizar del archivo soil_prop, la columan soil_prop$time que sea igual a t0)
...
hist(anova.org.matter.t0$residuals)
...
par(mfrow=c(2,2), cex.axis=1.5, lwd=2, font.lab=1.5) #para que los graficos se vean juntos
hist(lm.t0.mat.org$residuals)
plot(lm.t0.mat.org, which= 2)
...
```{r Phos, echo= FALSE}
phos.plot <- soil.summ%>% ggplot(aes(x=time, group= treatment, y= phosphorus, col= treatment))+
  geom_line()+
  geom_point()
phos.plot
```
...
```{r Calc, echo= FALSE}
calc.plot <- soil.summ%>% ggplot(aes(x=time, group= treatment, y= calcium, col= treatment))+
  geom_line()+
  geom_point()
calc.plot
```

...
```{r Materia organica plot, echo=FALSE}
...
phosp.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y=phosphorus_mean))+
  ...
  labs(title = "Figure 2: Phosphorus evolution over time .", subtitle = , caption = "Mean and sd of each treatment" )
phosp.plotsd
...
calcium.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= calcium_mean))+
  ...
  labs(title = "Figure 3: Calcium evolution over time .", subtitle = , caption = "Mean and sd of each treatment" )
calcium.plotsd
...
sodium.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= sodium_mean))+
  ...
  labs(title = "Figure 4: Sodium evolution over time .", subtitle = , caption = "Mean and sd of each treatment" )
sodium.plotsd
...
potassium.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= potassium_mean))+
  ...
  labs(title = "Figure 5: Potassium evolution over time .", subtitle = , caption = "Mean and sd of each treatment" )
potassium.plotsd
...
pH_water.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= ph.water_mean))+
  ...
  labs(title = "Figure 6: pH water evolution over time .", subtitle = , caption = "Mean and sd of each treatment" )
pH_water.plotsd
...
conductivity.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= conductivity_mean))+
  ...
  labs(title = "Figure 7: Conductivity evolution over time.", subtitle = , caption = "Mean and sd of each treatment" )
conductivity.plotsd
...
nitrogen.plotsd <- soil.summ.sd%>% ggplot(aes(x=time, group= treatment, col= treatment, y= nitrogen_mean))+
  ...
  labs(title = "Figure 8: Nitrogen evolution over time.", subtitle = , caption = "Mean and sd of each treatment" )
nitrogen.plotsd
...
plot_nitrogen_t4
```
## Differences between the beginning of the experiment (t0) and the end (t4)
...
anova_orgt0t4int <- aov(org.matter ~ time * treatment + rep, data=soilt0t4) #add rep= "block" To control for the effect of the differences between the 3 replicates # interaction between them?
summary(anova_orgt0t4int)

leveneTest(org.matter ~ time * treatment, data=soilt0t4) #equal variances 

tukey_orgmattert0t4time <- HSD.test(anova_orgt0t4int, "time", alpha = 0.05)$groups %>% rownames_to_column(var = "time")
tukey_orgmattert0t4time
tukey_orgmattert0t4treat <- HSD.test(anova_orgt0t4int, "treatment", alpha = 0.05)$groups %>% rownames_to_column(var = "treatment")
tukey_orgmattert0t4treat

avg_orgt0t4 <- aggregate(org.matter~ treatment:time, data=soilt0t4, FUN= mean)
#plot de treatment
#plot_orgmatter_t0t4 <- soilt0t4 %>% ggplot(aes(x= treatment, group= time, col=time, y= org.matter))+geom_point()+stat_summary(fun=mean, aes(x= treatment, y = org.matter),geom= "crossbar", colour= "violet", data=avg_orgt0t4, fatten=3)+geom_text(data=tukey_orgmattert0t4treat, aes(x=treatment, y = org.matter, label= groups), nudge_x= 0.2, nudge_y= 0.01)+labs(title = "Figure 1: Differences in org.matter between T0 and T4.", subtitle = "HSDTukey" , caption = "p-value" )

#plot_orgmatter_t0t4 #preguntar ignacio

cld <- multcompLetters4(anova_orgt0t4int, tukey_orgmattert0t4treat)
agvprueba<- aggregate(org.matter~ treatment, data=soilt0t4, FUN=mean)
prueba <- soilt0t4%>% ggplot(aes(x=treatment, group= time, col= time, y= org.matter))+ geom_point()+
  geom_text(data=tukey_orgmattert0t4treat, aes(x=treatment, y = org.matter, label= groups), nudge_x= 0.2, nudge_y= 0.01)+
  labs(title = "prueba at T4", subtitle = "HSDTukey" , caption = "p-value" )
  

prueba <- soilt0t4 %>%
  ggplot(aes(x=treatment, group= time, col= time, y= org.matter)) + 
  geom_point() +
...
nitrogen.plotsd <- soil.summ.sd%>% ggplot(aes(x=treatment, group= time, col= time, y= nitrogen_mean))+
  geom_line()+
  scale_colour_banana_disc("paleta.treat")+
  geom_point()+
  geom_errorbar(aes(ymin=nitrogen_mean-nitrogen_sd, ymax=nitrogen_mean+nitrogen_sd), width=0.2)+
...
anova_phosphot0t4 <- aov(phosphorus~ time + treatment, data=soilt0t4) #anova two ways
summary(anova_phosphot0t4)
anova_phosphot0t4int <- aov(phosphorus ~ time * treatment + rep, data=soilt0t4) #add rep= "block" To control for the effect of the differences between the 3 replicates # interaction between them?
summary(anova_phosphot0t4int)
leveneTest(phosphorus ~ time * treatment, data=soilt0t4) #equal variances 

tukey_phosphot0t4time <- HSD.test(anova_phosphot0t4int, "time", alpha = 0.05)$groups %>% rownames_to_column(var = "time")
tukey_phosphot0t4time
tukey_phosphot0t4treat <- HSD.test(anova_phosphot0t4int, "treatment", alpha = 0.05)$groups %>% rownames_to_column(var = "treatment")
tukey_phosphot0t4treat

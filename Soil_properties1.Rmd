...
avgm
plot.org.t01 <- soilt0 %>% ggplot(aes(x= treatment, y= org.mat.trans))+
  geom_point()+
  stat_summary(fun=mean, aes(x= treatment, y= org.mat.trans),
               geom= "crossbar", data=avgm)+
  geom_text(data=hsd.t0.org, aes(x=treatment, y=org.mat.trans, label= groups), nudge_x= 0.2, nudge_y= 3)
plot.org.t01
...
avgpho <- aggregate(trans.phospo~ treatment, data= soilt0, FUN= mean)
avgpho
plot.pho.t0 <- soilt0 %>% ggplot(aes(x= treatment, y= trans.phospo))+
  geom_point()+
  stat_summary(fun=mean, aes(x= treatment, y= trans.phospo),
               geom= "crossbar", data=avgpho)+
  geom_text(data=hsd.t0.pho,aes(x=treatment, y= trans.phospo, label= groups), nudge_x= 0.2, nudge_y= 0.2)

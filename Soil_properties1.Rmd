...
anova(orglm)
emmip(orglm, time ~ treatment)#to visualize the interactions 

emm_org <- emmeans(orglm, pairwise ~ time | treatment, adjust= "tukey")#how to add tukey
emm_org$contrasts %>% rbind(adjust= "fdr") %>% 
  summary(infer = TRUE)
emm_org <- cld(emmeans(orglm, specs = pairwise~treatment), Letters= letters, reversed= TRUE)

d1 <- data.frame(emm_org$contrasts)
d2 <- data.frame(emm_org$emmeans) %>% group_by(treatment) %>% summarise(across(.cols= "emmean",.fns=mean),.groups = "drop")
str(d1)

pval.org.mat.time <- data.frame(treatment= d1$treatment, pval= d1$p.value, emmean= d2$emmean)
...
pairs(emm_org)
summary(pairs(emm_org), by =NULL, adjust="tukey")
plot(emm_org, comparison= TRUE)


#pairs(emm_org, adjust= "Tukey")#to add tukey?
#summary(pairs(emm_org), by =NULL adjust="tukey")#If you wish to apply a P-value adjustment to all tests across all groups, you need to null-out the by variable and summarize, as in the following. 
plot(emm_org, comparison= TRUE) #to see the comparison

pwpp(emm_org)#Pairwise P-value plots

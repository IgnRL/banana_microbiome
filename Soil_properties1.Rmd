...
emm_org$contrasts %>% rbind(adjust= "fdr") %>% #el adjust es para el metodo de los pvalues
  summary(infer = TRUE)
emm_org <- cld(emmeans(orglm, specs = pairwise~treatment), Letters= letters, reversed= TRUE) #tabla con las diferencias sig y las letras se lo indicas 
emm_org #tabla con los resultados. Se hacen dataframe para sacar las columnas que queremos y poder unirlas en los graficos 

orgcontras <- data.frame(emm_org$contrasts)#creamos data frame para sacar el valor del contraste 
...
#para hacer el delta, es decir cuanto ha variado la materia organica. 

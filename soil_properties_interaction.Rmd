...
model_orgt0t4 <- lmerTest:lmer(org.matter~ time*treatment + (1|treat.pot), data=soilt0t4) 
#model time + (1+ time|treatment)
...
anova_orgt0t4int<- summary(aov(org.matter ~ time * treatment +rep, data=soilt0t4))
pvalue_anova_orgt0t4int<- anova_orgt0t4int[[1]][["Pr(>F)"]][1]




#lmerTest::lmer(org.matter~ time+( 1| treatment), data = soilt0t4)
...
orgmattert0t4_time <- soil_t0t4_mean %>% ggplot(aes(x=factor(treatment, level= treat.order), y= org.matter_mean), col=time)+
  ...
  labs(title = "", subtitle = "", caption = "Different (*) indicate statistically significant differences between treatments at T0 and T4.\nData comes to Emmeans of each treatment in these time and SD", size = 10)+
  ggtitle("OM (%) in T0 and T4", subtitle= paste("ANOVA two was interact -Time- (HSDTukey) p-value: ", format(pvalue_anova_orgt0t4int, scientific= TRUE, digits=2 ), sep= ""))+
...
delta.mat.org
delta.org.lmer <- lmer(delta~ time + (1+ time|treatment), data=delta.mat.org)#mixed-effects model,
